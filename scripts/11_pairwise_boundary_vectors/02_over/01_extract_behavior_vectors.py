...
def parse_args():
    ...
    parser.add_argument(
        "--activation-cache",
        default="outputs/11_pairwise_boundary_vectors/00_cache/answer_mean_activations_kall.pt",
    )
    parser.add_argument(
        "--output-dir",
    ...
    parser.add_argument(
        "--compose-under",
        action="store_true",
        help="Also write the reverse composed axis for under steering.",
    )
...
def main():
    ...
    label_counts = print_label_distribution(rows, "pred_label")
    ...
    for pos_label, neg_label in LABEL_PAIRS:
        ...
        payload = build_payload(
            ...
            group_name=group,
            definition={
                "activation": "mean hidden state over teacher-forced answer tokens",
                "positive": f"rows the model answered {pos_label}",
                "negative": f"rows the model answered {neg_label}",
        ...
        save_payload(payload, output_dir / f"behavior_vectors_{group}.pt")

    compose_axis(
        pair_payloads,
        args,
        vector_key="behavior_vector",
        output_path=output_dir / "behavior_vectors_pairwise_axis_over.pt",
        side="over",
    )
    if args.compose_under:
        compose_axis(
            ...
            args,
            vector_key="behavior_vector",
            output_path=project_path("outputs/11_pairwise_boundary_vectors/03_under/vectors/under_behavior_vectors_pairwise_axis.pt"),
            side="under",
        )
