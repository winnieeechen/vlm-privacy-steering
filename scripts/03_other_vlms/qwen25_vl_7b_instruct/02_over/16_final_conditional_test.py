...
def main():
    ...
    parser.add_argument(
        "--base-csv",
        default=(
            "outputs/03_other_vlms/qwen25_vl_7b_instruct/00_base/test/"
            "base_qwen25_vl_7b_test_243.csv"
        ),
    )
    parser.add_argument(
        "--steered-csv",
        default=(
            "outputs/03_other_vlms/qwen25_vl_7b_instruct/02_over/test/"
            "steered_qwen25_vl_7b_test_layer32_alpha0.5.csv"
        ),
    )
    parser.add_argument(
        "--score-csv",
        default=(
            "outputs/03_other_vlms/qwen25_vl_7b_instruct/02_over/test/"
            "condition_scores_test_layer32.csv"
        ),
    )
    parser.add_argument(
        "--output-csv",
        default=(
            "outputs/03_other_vlms/qwen25_vl_7b_instruct/02_over/test/"
            "final_conditional_test_layer32_alpha0.5_thr-0.03.csv"
        ),
    )
    ...
    print_metrics(
        "Unconditional steered",
        metrics(steered_rows, "steered_pred_label", "steered_case_type"),
    )
    print_metrics(
        "Final conditional",
        metrics(results, "conditional_pred_label", "conditional_case_type"),
    )
    print("\nSaved:", out_path)
