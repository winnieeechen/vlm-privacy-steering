...
DEFAULT_OVER_THRESHOLD = -0.03
DEFAULT_UNDER_THRESHOLD = 0.04
...
def paths_for_split(split, over_layer, over_alpha, under_layer, under_alpha):
    n = {"val": 238, "test": 243}[split]
    base = ROOT / (
        f"outputs/03_other_vlms/qwen25_vl_7b_instruct/00_base/{split}/"
        f"base_qwen25_vl_7b_{split}_{n}.csv"
    )
    over_steered = ROOT / (
        f"outputs/03_other_vlms/qwen25_vl_7b_instruct/02_over/{split}/"
        f"steered_qwen25_vl_7b_{split}_layer{over_layer}_alpha{over_alpha}.csv"
    )
    under_steered = ROOT / (
        f"outputs/03_other_vlms/qwen25_vl_7b_instruct/03_under/{split}/"
        f"under_steered_qwen25_vl_7b_{split}_layer{under_layer}_alpha{under_alpha}.csv"
    )
    over_scores = ROOT / (
        f"outputs/03_other_vlms/qwen25_vl_7b_instruct/02_over/{split}/"
    ...
    out_dir = ROOT / f"outputs/03_other_vlms/qwen25_vl_7b_instruct/04_combined_router/{split}"
...
def main():
    ...
    parser.add_argument("--over-alpha", default="0.5")
    parser.add_argument("--under-layer", type=int, default=32)
    parser.add_argument("--under-alpha", default="0.5")
    parser.add_argument("--over-threshold", type=float, default=DEFAULT_OVER_THRESHOLD)
    parser.add_argument("--under-threshold", type=float, default=DEFAULT_UNDER_THRESHOLD)
    ...
    print("n =", n)
    print("over layer/alpha =", args.over_layer, args.over_alpha)
    print("under layer/alpha =", args.under_layer, args.under_alpha)
    ...
    for strategy in strategies:
        ...
        tag = (
            f"over{args.over_layer}_alpha{args.over_alpha}_"
            f"under{args.under_layer}_alpha{args.under_alpha}"
        )
        out_path = out_dir / f"combined_{args.split}_{strategy}_{tag}.csv"
    ...
    summary_path = out_dir / f"combined_{args.split}_summary_{tag}.csv"
