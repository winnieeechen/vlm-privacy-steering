...
COND_PATH = ROOT / "outputs" / "vectors" / "condition_vectors_qwen_vl_train.pt"
BEHAV_PATH = ROOT / "outputs" / "vectors" / "behavior_vectors_qwen_vl_train.pt"
...
def main():
    ...
    for layer, norm in topk_layers(c_norms):
        print(f" layer {layer:02d}: norm={norm:.4f}")

    print("\nTop behavior-vector layers by norm:")
    for layer, norm in topk_layers(v_norms):
        print(f" layer {layer:02d}: norm={norm:.4f}")
    ...
    for i, val in zip(abs_indices, abs_values):
        ...
        print(
            ...
            f"c_norm={float(c_norms[i]):.4f}, "
            f"v_norm={float(v_norms[i]):.4f}"
        )
